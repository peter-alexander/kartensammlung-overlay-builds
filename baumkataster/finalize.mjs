...
const DEFAULT_PUBLISH_DIR = path.resolve("baumkataster/build/Baumkataster");
...
function assertRelease(release) {
	...
	if (!vectorTiles || vectorTiles.layer !== "baumkataster") {
		throw new Error("release.json has no Baumkataster vector-tile definition.");
	}
	if (
		Number(vectorTiles.minzoom) !== 12
		|| Number(vectorTiles.maxzoom) !== 15
	) {
		throw new Error("Unexpected Baumkataster zoom range.");
	}
	if (
...
async function main() {
	...
	for (let zoom = 12; zoom <= 15; zoom += 1) {
		...
		if (!keys.length) {
			throw new Error(`No generated PBF tiles found for Z${zoom}.`);
		}
	...
	console.log(
		`Finalized Baumkataster manifests: Z12=${tileKeysByZoom[12].length}, `
